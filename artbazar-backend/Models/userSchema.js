...
userMongooseSchema.statics.signUp = async function (data) {
    ...
    if (exists) {
        throw new CustomError('Email', "Account with this email has already been registered",)
    }

    const hashedPass = await bcrypt.hash(password, 10)

    const token = generateRandomToken()
    const user = await this.create({ name, userName, email, password: hashedPass, emailConfirmationToken: token })
    const mailOptions = {
        to: email,
...
userMongooseSchema.statics.forgetPasswordUpdate = async function (data) {
    ...
    if (exists) {
        if (exists.isPasswordResetTokenVerified) {
            ...
            throw Error("Please verify password reset token before changing password")
        }
    } else {
        throw Error("User with this email doesn't exists")
    }
