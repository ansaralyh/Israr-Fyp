...
const landingcard = ({ user, handleDelete, art, landingimg, landingheader, profilepic, username, userdate }) => {
      ...
      <div className="landing-card">
        <div className="landing-card-top">
                    ...
                    <>
                      <div className="card-edit-del">
                        ...
                        <div className="edit-del-circle">
                          ...
                          <TbTrash onClick={() => handleDelete(art._id)} className='landing-card-icon' />
          ...
          <Link to={`/post/${art?._id}`}>
            <div className="landing-card-image">
              <img style={{ width: '350px', height: '258px' }} className='landing-card-img' src={art?.imageUrl} alt="" />
            </div>
        ...
        <Link to={'/profile'}>
          <div className="profile-link">
            <img className='landing-card-proimg' style={{ borderRadius: '50%' }} src={art?.user_id?.profileUrl} alt="" />
            <p className="user-name">{art?.user_id?.userName}</p>
            <div className="usergap"></div>
            <p className="user-date">{new Date(art?.publishedDate).toDateString()}</p>
          </div></Link>
